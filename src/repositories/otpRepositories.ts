...
export class OtpRepository {
    static async create(email: string, code: string, expires_at: Date , type: 'email_verification' | 'password_reset'): Promise<Otp> {
    ...
    static async findValidOTP(email: string , type: 'email_verification' | 'password_reset'): Promise<Otp | null> {
        const result = await pool.query(
          `SELECT * FROM otps 
    ...
    static async findByEmail(email: string , type: 'email_verification' | 'password_reset'): Promise<Otp | null> {
        const result = await pool.query(
    ...
    static async deleteMany(email: string , type: 'email_verification' | 'password_reset'): Promise<void> {
        await pool.query(`DELETE FROM otps WHERE email = $1 AND type = $2`, [email , type]);
    }

    static async update(email: string, code: string, expires_at: Date, used: boolean , type :'email_verification' | 'password_reset'): Promise<Otp> {
        const result = await pool.query(
